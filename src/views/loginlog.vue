  ...
  <div class="loginlog">
    ...
    <div class="layout-box-right-bottom">
      <el-table :data="tableData" style="width: 100%" height="100%">
        <el-table-column prop="loginMember" label="登录成员" width="220" />
        <el-table-column prop="loginTime" label="登录时间" />
        <el-table-column prop="state" label="状态">
          <template #default="{ row }">
            <span v-if="row.state === '1'" class="login">正常登录</span>
            <span v-else class="quit">用户退出</span>
          </template>
        </el-table-column>
        <el-table-column prop="localNetwork" label="本机网络" />
        <el-table-column prop="place" label="地点" />
        <el-table-column prop="terminalCode" label="终端识别码" width="350" />
